...
<template>
  <main class="flex items-center justify-center min-h-[calc(100vh-73px)]">
    <div class="w-full max-w-sm mx-4">
      ...
      <div v-if="displayError" class="mb-6 px-4 py-3 rounded-lg bg-negative/10 border border-negative/30 text-negative text-sm">
        {{ displayError }}
      </div>

      <!-- Forgot Password form -->
      <template v-if="showForgotPassword">
        <div v-if="forgotSuccess" class="mb-6 px-4 py-3 rounded-lg bg-positive/10 border border-positive/30 text-positive text-sm">
          Check your email for a reset link.
        </div>
        <form v-else class="space-y-4 mb-6" @submit.prevent="handleForgotPassword">
      ...
      <div class="flex items-center gap-3 mb-6">
        <div class="flex-1 h-px bg-surface-lighter" />
