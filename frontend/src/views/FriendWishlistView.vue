...
const { games, friendInfo, loading, error, fetchFriendWishlist } = useFriendGames()
...
onMounted(() => {
  fetchFriendWishlist(Number(route.params.userId))
})
</script>
  ...
  <main class="max-w-7xl mx-auto px-4 py-6">
    ...
    <div class="flex items-center gap-3 mb-6">
      ...
      <h1 class="text-3xl font-bold text-text-primary">
        <template v-if="friendInfo">{{ friendInfo.name }}'s Wishlist</template>
        <template v-else>Wishlist</template>
      </h1>
    </div>
